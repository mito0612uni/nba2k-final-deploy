...
def analyze_stats_image():
    ...
    # ==========================================
    prompt_text = """
    あなたはバスケットボールのスタッツ集計AIです。
    提供された【全ての画像】から、見えている【全ての選手行】を読み取り、JSONデータに変換してください。

    【最重要：合算禁止・重複維持】
    - **絶対に数値を足し算しないでください。**
    - 同じ選手が複数の画像に出てくる場合も、**省略せず、そのまま別々のデータとして出力してください。**
      (後の工程でプログラムが合算するため、あなたは見たままを出力する必要があります)

    【重要なヒント：表の列構成について】
    この画像の表は、左から順に以下の構成になっています：
    1. **選手名** (例: IIT-ZERO-KII)
    2. **チームメイト評価** (例: A, A+, B- など) ← ★この列は無視してください！数値ではありません。
    3. **PTS** (得点)
    4. **REB** (リバウンド)
    5. **AST** (アシスト)
    6. **STL** (スティール)
    ...
    【指示】
    - 一番上の行（黄色や白のハイライト行）も必ずデータとして抽出してください。
    - 「評価(Grade)」の列が数値の邪魔をしないように注意してください（PTSと混同しないこと）。
    - debug_raw_text には読み取ったそのままの行を含めてください。

    出力フォーマット（JSONのみ）:
    {
        "debug_raw_text": [ "読み取った行1", "読み取った行2", ... ],
        "players": [
            {
                "name": "選手名",
