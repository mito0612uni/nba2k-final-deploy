...
def analyze_stats_image():
    ...
    try:
        ...
        【最重要ルール: 順序の厳守】
        画像の上から下へ、視覚的な行の並び順（Y座標順）を【絶対に】崩さずに抽出してください。
        ・チームごとに分かれている場合も、画像の「上にあるチーム」の選手から先にリストアップし、次に「下にあるチーム」の選手を続けてください。
        ・AIの判断で名前順や得点順に並び替えることは【禁止】です。
        ・画像の見た目通りの順序で出力されているか、最後に必ず確認してください。
        
        その他のルール:
        1. 画像内に表示されている【全ての行（選手）】を漏らさず抽出してください。
        2. 全ての選手を1つの "players" リストにまとめてください。
