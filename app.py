...
def calculate_standings(league_filter=None):
    ...
    for team in teams:
        ...
        for game in home_games:
            if game.winner_id is None: # 引き分けまたは未判定だがis_finished=Trueの場合
...
def calculate_team_stats():
    ...
    # 修正: 終了した試合(is_finished=True)のみを集計するようにGameと結合してフィルタリング
    ...
    for team_standings in standings_info:
        ...
        # デフォルト値の設定
        stats_dict.update({
            'avg_ast': 0, 'avg_reb': 0, 'avg_stl': 0, 'avg_blk': 0,
            'avg_foul': 0, 'avg_turnover': 0, 'fg_pct': 0, 'three_p_pct': 0, 'ft_pct': 0
        })

        if stats_games_played > 0 and team_shooting:
            # None対策として (x or 0) を使用
            total_ast = team_shooting.total_ast or 0
            total_reb = team_shooting.total_reb or 0
            total_stl = team_shooting.total_stl or 0
            total_blk = team_shooting.total_blk or 0
            total_foul = team_shooting.total_foul or 0
            total_turnover = team_shooting.total_turnover or 0
            
            total_fgm = team_shooting.total_fgm or 0
            total_fga = team_shooting.total_fga or 0
            total_3pm = team_shooting.total_3pm or 0
            total_3pa = team_shooting.total_3pa or 0
            total_ftm = team_shooting.total_ftm or 0
            total_fta = team_shooting.total_fta or 0

            stats_dict.update({
                'avg_ast': total_ast / stats_games_played,
                'avg_reb': total_reb / stats_games_played,
                'avg_stl': total_stl / stats_games_played,
                'avg_blk': total_blk / stats_games_played,
                'avg_foul': total_foul / stats_games_played,
                'avg_turnover': total_turnover / stats_games_played,
                'avg_fgm': total_fgm / stats_games_played,
                'avg_fga': total_fga / stats_games_played,
                'avg_three_pm': total_3pm / stats_games_played,
                'avg_three_pa': total_3pa / stats_games_played,
                'avg_ftm': total_ftm / stats_games_played,
                'avg_fta': total_fta / stats_games_played,
...
def mvp_selector():
    ...
    if request.method == 'POST':
        ...
        if action == 'calculate' or action == 'publish':
                ...
                if action == 'calculate':
                    ...
                    def save_top_players(league_name):
                        query = db.session.query(
                         ...
                         .join(Team, Player.team_id == Team.id)\
                         .join(Game, PlayerStat.game_id == Game.id)\
                         .filter(Game.game_date >= start_date_str)\
                         .filter(Game.game_date <= end_date_str)\
                         .filter(Team.league == league_name)\
                         .group_by(Player.id, Team.id)\
                         .having(func.count(PlayerStat.game_id) >= 1)\
                         .order_by(db.desc('score')).limit(5)
            ...
            setting = SystemSetting.query.get('show_mvp')
            if not setting:
                setting = SystemSetting(key='show_mvp', value=new_val); db.session.add(setting)
...
def roster():
    if request.method == 'POST':
        ...
        if action == 'add_team':
            ...
            title = request.form.get('news_title'); content = request.form.get('news_content')
            if title and content:
                new_item = News(title=title, content=content); db.session.add(new_item); db.session.commit(); flash(f'お知らせ「{title}」を投稿しました。')
...
def team_detail(team_id):
    ...
    # 解析対象のフィールド定義
    team_fields = {
        'avg_pf': {'label': '平均得点'}, 'avg_pa': {'label': '平均失点', 'reverse': True},
        'diff': {'label': '得失点差'}, 'fg_pct': {'label': 'FG%'},
        'three_p_pct': {'label': '3P%'}, 'ft_pct': {'label': 'FT%'},
        'avg_reb': {'label': 'リバウンド'}, 'avg_ast': {'label': 'アシスト'},
        'avg_stl': {'label': 'スティール'}, 'avg_blk': {'label': 'ブロック'},
        'avg_turnover': {'label': 'ターンオーバー', 'reverse': True}, 'avg_foul': {'label': 'ファウル', 'reverse': True},
    }
    
    analyzed_stats = analyze_stats(team_id, all_team_stats_data, 'none', team_fields)

    return render_template('team_detail.html', team=team, player_stats_list=player_stats_list, team_games=team_games, team_stats=target_team_stats, stats=analyzed_stats)
